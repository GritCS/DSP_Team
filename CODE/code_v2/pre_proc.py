...
from util_tool import gaussian_radius, draw_umich_gaussian
...
def generate_ground_truth(image,
    ...
    for k in range(17):
        pts = pts_2[4*k:4*k+4,:]  #一个骨块的四个顶点，pts.shape=(4,2)
        ...
        ct_int = ct.astype(np.int32) #一个骨块的中心的x,y坐标  shape:（2,）
        radius = gaussian_radius((math.ceil(bbox_h), math.ceil(bbox_w)))
        radius = max(0, int(radius))#生成的高斯分布的半径
        draw_umich_gaussian(hm[0,:,:], ct_int, radius=radius)
        ind[k] = ct_int[1] * image_w + ct_int[0]    #ct_int[1]:y坐标，按照行优先的准则，骨块中点所处的像素点的下标

        reg[k] = ct - ct_int # 关键点和坐标index之间的细微差别
        reg_mask[k] = 1

        for i in range(4):
            wh[k,2*i:2*i+2] = ct-pts[i,:]#pts.shape=(4,2)，pts[i,:]表示第i个顶点  wh[k,2*i:2*i+2]得到的是中心指向顶点的方向向量
